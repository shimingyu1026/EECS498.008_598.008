...
def svm_loss_vectorized(W, X, y, reg):
    ...
    # 2. 获取每个样本正确分类的分数，并调整其形状以用于广播
    correct_class_scores = scores[torch.arange(num_train), y].view(num_train, 1)

    # 3. 计算间隔（margin）
    margins = scores - correct_class_scores + 1  # 利用广播机制

    # 4. 应用 max(0, ...) 函数
    margins = margins.clamp(min=0)

    # 5. 将正确分类位置的 margin 设置为 0
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # 1. 创建一个二元掩码，其中 margin > 0 的位置为 1
    binary_mask = torch.zeros_like(margins)
    binary_mask[margins > 0] = 1

    # 2. 计算每个样本有多少个类别产生了损失
    num_pos_margins = binary_mask.sum(dim=1)

    # 3. 在正确分类的位置，设置为负的产生损失的类别数量
    binary_mask[torch.arange(num_train), y] = -num_pos_margins

    # 4. 计算梯度
    dW = X.t().mm(binary_mask) / num_train
...
def train_linear_classifier(
    ...
    if W is None:
        ...
        W = 0.000001 * torch.randn(
            dim, int(num_classes), device=X.device, dtype=X.dtype
        )
    else:
        num_classes = W.shape[1]
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.25e-2, 0.75e-2, 1e-2, 1.25e-2, 1.75e-2]
    regularization_strengths = [0.75e-6, 1e-6, 1.25e-6]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(cls: LinearSVM, data_dict, lr, reg, num_iters=2000):
    """
    ...
    # Replace "pass" statement with your code
    cls.train(
        data_dict["X_train"], data_dict["y_train"], lr, reg, num_iters, batch_size=512
    )
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    scores = X @ W
    stable_scores = scores - torch.max(scores, dim=1, keepdim=True).values
    ...
    data_loss = -torch.log(correct_class_probs).mean()
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.4e-2, 1.5e-2,1.6e-2, 1.7e-2]
    regularization_strengths = [1e-5, 1e-6, 1e-4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
