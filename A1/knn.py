...
def compute_distances_two_loops(x_train, x_test):
    ...
    for i in range(num_train):
        flattened_train = x_train[i].flatten()
        for j in range(num_test):
            flattened_test = x_test[j].flatten()
            dist = torch.sum((flattened_train - flattened_test) ** 2)
            dists[i, j] = dist
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def compute_distances_one_loop(x_train, x_test):
    ...
    for i in range(num_train):
        flattened_train = x_train[i].flatten()
        flattened_test = x_test.reshape(num_test, -1).clone()
        dist = torch.sum((flattened_test - flattened_train) ** 2, dim=1)
        dists[i, :] = dist
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def compute_distances_no_loops(x_train, x_test):
    ...
    sq_x_train = torch.sum(x_train_flat**2, dim=1).unsqueeze(1)

    sq_x_test = torch.sum(x_test_flat**2, dim=1).unsqueeze(0)

    matmul_term = -2 * torch.matmul(x_train_flat, x_test_flat.T)

    dists = sq_x_train + sq_x_test + matmul_term
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return dists
...
def predict_labels(dists, y_train, k=1):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    index = torch.topk(dists, k, dim=0, largest=False)[1]
    labels = y_train[index]
    y_pred = torch.mode(labels, dim=0)[0]
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_train_data = torch.cat(x_train_folds[:i] + x_train_folds[i + 1 :])
            y_train_data = torch.cat(y_train_folds[:i] + y_train_folds[i + 1 :])
            knn = KnnClassifier(x_train_data, y_train_data)
            k_to_accuracies[k].append(
                knn.check_accuracy(x_train_folds[i], y_train_folds[i], k)
            )
    ##############################################################################
    #                            END OF YOUR CODE                                #
    ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  """
    ...
    for k in sorted(k_to_accuracies.keys()):
        ...
        # If the current k's average accuracy is better than the best we've seen,
        # update our records.
